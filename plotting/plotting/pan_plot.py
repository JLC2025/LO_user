...
python pan_plot.py -gtx cas2k_v4_x2b -ro 0 -0 1993.01.15 -lt snapshot -pt P_Chl_DO -avl True -mtag Temp -save True

python pan_plot.py -gtx cas2k_v4_x2b -ro 0 -0 1993.01.01 -1 1994.12.31 -lt daily -pt P_layer -avl True -mtag DIC_Alk_10m -mov True
...
# which run to use
parser.add_argument('-gtx', '--gtagex', default='cas2k_v4_x2b', type=str)
parser.add_argument('-ro', '--roms_out_num', default=0, type=int)
# 2 = Ldir['roms_out2'], etc.

# select time period and frequency
parser.add_argument('-0', '--ds0', default='1993.01.01', type=str)
...
parser.add_argument('-hn', '--his_num', default=9, type=int)
...
if len(fn_list) == 1:
    ...
    if Ldir['save_plot'] == True:
        in_dict['fn_out'] = outdir0 / Ldir['list_type'] / (Ldir['list_type'] + '_'
            + Ldir['plot_type'] + '_' + Ldir['gtagex'] + '_' + Ldir['movie_tag'] + '_' + Ldir['ds0'] + '.png')
    else:
        in_dict['fn_out'] = ''
    whichplot(in_dict)
    ...
    if Ldir['make_movie']:
        ...
        # cmd_list = ['ffmpeg','-r','8','-i', str(outdir)+'/plot_%04d.%02d.%02d.png', '-vcodec', 'libx264',
        #     '-pix_fmt', 'yuv420p', '-crf', '25', str(outdir)+'/' + Ldir['movie_name'] + '.mp4']
