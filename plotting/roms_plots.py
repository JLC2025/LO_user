...
python pan_plot.py -gtx cas2k_v4_x2b -ro 0 -0 1993.01.01 -1 1994.12.31 -lt daily -pt P_layer -mov True

run pan_plot.py -gtx cas2k_v4_x2b -ro 0 -0 2001.01.15 -lt snapshot -pt P_Chl_DO -avl False -mtag temp_bot -save True
"""
import numpy as np
import xarray as xr
import pickle
...
def P_basic(in_dict):
    ...
    pfun.start_plot(fs=fs, figsize=(7,10)) # now change it to single plot
    fig = plt.figure()
    # PLOT CODE
    #vn_list = ['salt', 'temp']
    vn_list = ['salt']
...
def P_Chl_DO(in_dict):
    ...
    # vn_list =['LdetritusN']     
    # vn_list = ['chlorophyll']  
    # vn_list =['phytoplankton'] 
    # vn_list =['zooplankton']  
    
    fs = 14
    ii = 1
    for vn in vn_list:
        if vn == vn_list[0]: #'salt':
            slev = -1
            stext = 'Surface'
            # slev = 0
            # stext = 'Bottom'
        if vn == 'NH4':
            pinfo.cmap_dict[vn] = cmr.hawaii_r
        # elif vn == 'oxygen':
            # slev = 0
            # stext = 'Bottom'
            # slev = -1
        ...
        cs = pfun.add_map_field(ax, ds, vn, pinfo.vlims_dict, slev=slev,
                cmap=pinfo.cmap_dict[vn], fac=pinfo.fac_dict[vn],
                vlims_fac=pinfo.range_dict[vn], do_mask_edges=False)
        # add velocity vector

        if slev == -1:
            pfun.add_velocity_vectors(ax, ds, in_dict['fn'], zlev='top') 
            pfun.add_windstress_flower(ax, ds)
            # pfun.add_info(ax, in_dict['fn'])    
        if slev == 0:
            pfun.add_velocity_vectors(ax, ds, in_dict['fn'], zlev='bot') 
            # mask any depth < -200
...
def P_layer(in_dict):
    ...
    pfun.start_plot(fs=fs, figsize=(7,10)) # for one variable
    fig = plt.figure()
    ds = xr.open_dataset(in_dict['fn'])
    # PLOT CODE
    import cmcrameri.cm as cmr
    ...
    for vn in vn_list:
        ...
        pfun.dar(ax)
        ax.set_xlabel('Longitude')
        ax.set_title('%s %s at Z = %d (m)' % (pinfo.tstr_dict[vn], pinfo.units_dict[vn], z_level))
        if ii == 1:
            pfun.add_info(ax, in_dict['fn'])
            ax.set_ylabel('Latitude')
            # pfun.add_windstress_flower(ax, ds)
...
def P_DO_WA_shelf(in_dict):
    ...
    # aa = [-130, -122, 42, 52]
    # xtl = [-129, -128, -127,-126, -125, -124, -123]
    # ytl = [44, 46, 48, 50]
    ...
    # ax.set_xticks(xtl)
    # ax.set_yticks(ytl)
    pfun.add_info(ax, in_dict['fn'], loc='lower_right')
    
    # pfun.add_windstress_flower(ax, ds, t_scl=0.5, t_leglen=0.1, center=(.85,.65), fs=12)
    pfun.add_windstress_flower(ax, ds)
    # pfun.add_velocity_vectors(ax, ds, in_dict['fn'], zlev=slev)
...
def P_sect_hc(in_dict):
    ...
    run pan_plot.py -gtx cas2k_v4_x2b -ro 0 -0 2009.08.11 -lt snapshot -pt P_sect_hc -avl False -mtag temp -save True
    """
    # START
    fs = 14
    pfun.start_plot(fs=fs, figsize=(20,9))
    ...
    if vn == 'salt':
        ...
        pinfo.cmap_dict[vn] = 'jet' # this is only for comparing with SXS 2009
    # GET DATA
    G, S, T = zrfun.get_basic_info(in_dict['fn'])
    # CREATE THE SECTION
    # read in a section (or list of sections)
    ...
    # fig.colorbar(cs, ax=ax) # It is identical to that of the section
    pfun.add_coast(ax)
    # aaf = [-123.5, -122.5, 47.25, 48.5] # focus domain hc
    aaf = [-128, -123, 42, 48] # focus domain hc
    ax.axis(aaf)
    pfun.dar(ax)
    pfun.add_info(ax, in_dict['fn'], loc='upper_right')
    ax.set_title('Surface %s %s' % (pinfo.tstr_dict[vn],pinfo.units_dict[vn]))
...
def P_sect_coast(in_dict):
    ...
    # START
    fs = 14
    pfun.start_plot(fs=fs, figsize=(16,15))
    fig = plt.figure()
    ds = xr.open_dataset(in_dict['fn'])
    # PLOT CODE
    ...
    for vn in vn_list:
        ...
        tracks_path = Ldir['LOo'] / 'section_lines'
        # tracks = ['hc1.p','hc2.p','hc3.p']
        tracks = ['Heceta_Bank.p']  # 2009.08.08  
        # tracks = ['NHL.p'] # 2009.08.10 
        # tracks = ['Cape_Perpetua.p'] # 2009.08.09
        # tracks = ['Cascade_Head.p'] # 2009.08.07 2009.08.06 2009.07.31 2009.07.30 

        if tracks == ['Cascade_Head.p']:
            zdeep = -200
        else:
            zdeep = -220
...
def P_splash(in_dict):
    ...
    run pan_plot.py -gtx cas2k_v4_x2b -ro 0 -0 1999.01.01 -1 1999.12.31 -lt daily -pt P_splash -avl False -mov True -save True
    """
    # START
    fs = 14
    ...
    pfun.dar(ax)
    ax.axis(aa)
    ax.set_xticks([-125, -124.5, -124])
    ax.set_yticks([46, 47, 48, 49])
    ax.set_xlabel('Longitude')
    ax.text(.85,.99,'Bottom water\nAragonite\nSaturation\nState',
         ha='right', va='top', weight='bold', transform=ax.transAxes)
...
def P_superplot_oxygen_coast(in_dict):
    ...
    first need to run 
    run pan_plot.py -gtx cas2k_v4_x2b -ro 0 -0 1993.01.01 -1 1993.12.31 -lt daily -pt P_superplot_oxygen_coast -mov True -save True
    ...
    # yy = np.array([47, 47])    # Gray's Habor
    xx = np.array([-124.7, -124.125])
    yy = np.array([44.652, 44.652]) # NHL
    for ii in range(len(xx)-1):
        x0 = xx[ii]
        x1 = xx[ii+1]
        y0 = yy[ii]
    ...
    ax.set_axis_off()
    # add a box for the subplot
    aa = [-125.8, -123.5, 46, 49] # WA
    aa = [-125.8, -123.5, 43.5, 45] # OR
    # aa = [-123.5, -122.1, 47.03, 48.8]
    pfun.draw_box(ax, aa, color='c', alpha=.5, linewidth=5, inset=.01)
    # labels
    ax.text(.95, .07, 'LiveOcean\nBottom Oxygen\n'
    ...
    cs = ax.pcolormesh(dist_se, zw_se, sf,
                       vmin=vlims3[0], vmax=vlims3[1], cmap=cmap)
    # labels
    ax.text(.6, .3, 'SECTION\nNHL', fontsize=fs, color='b',ha='center',
        transform=ax.transAxes)
    ax.set_axis_off()
...
def P_superplot_salt(in_dict):
    ...
    # read in a section (or list of sections)
    tracks_path = Ldir['data'] / 'section_lines'
    # tracks = ['Line_ps_main_v0.p'] # Puget sound
    tracks = ['NHL.p']
    zdeep = -250
    xx = np.array([])
    yy = np.array([])
    ...
    ax.set_axis_off()
    # add a box for the subplot
    aa = [-123.5, -122.1, 47.03, 48.8] # PS
    aa = [-123.8, -125.2, 43, 46] # OR
    pfun.draw_box(ax, aa, color='c', alpha=.5, linewidth=5, inset=.01)
    # labels
    ax.text(.95, .07, 'LiveOcean\nSalinity\n'
        + datetime.strftime(T['dt'], '%Y'), fontsize=fs, color='k',
...
def P_superplot_NH4(in_dict):
    ...
    vn = 'NH4'
    vlims = (0, 1) # full map
    vlims2 = (0, 1) # regional map
    vlims3 = (0, 8) # regional section
    cmap = 'Spectral_r'
...
def P_superplot_NO3(in_dict):
    ...
    vn = 'NO3'
    vlims = (0, 25) # full map
    vlims2 = (0, 30) # regional map
    vlims3 = (0, 35) # regional section
    cmap = 'Spectral_r'
...
def P_superplot_temp(in_dict):
    ...
    vn = 'temp'
    vlims = (5, 25) # full map
    vlims2 = (5, 25) # regional map
    vlims3 = (7, 9) # regional section
    cmap = 'Spectral_r'
